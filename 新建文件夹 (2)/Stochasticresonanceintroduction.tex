...
\caption{Figure:Typical characteristic curve of random resonance}
\label{fig:lable}
\end{figure}
\par
It can be seen in the diagram that with the increase of the noise intensity, the signal to noise ratio of the output increases greatly, and the peak value shows that there is a transfer of noise energy to the periodic signal.
\par
...
\par
\section{TOOL}
The first evidence of stochastic resonance was produced by simulating the Budyko-Sellers model of climate change (Benzi et al., 1982) on a Digital Instruments mainframe computer (model PDP 1000), an advanced computer at the time.
\par
...
we shall consider a Brownian particle of mass m that moves in a bistable potential V(x) and is subjected to thermal noise j (t) of the Nyquist type at temperature T. we perturb the particle with a periodically varying force.
\bibliography{abc}
\end{document}
